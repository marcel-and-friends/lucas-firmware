...
#include "../platforms.h"
    ...
    #endif
    #ifndef LUCAS_TIMER
        #define LUCAS_TIMER MCU_LUCAS_TIMER
    #endif
    ...
    #define LUCAS_TIMER_DEV _TIMER_DEV(LUCAS_TIMER)
...
void HAL_timer_start(const uint8_t timer_num, const uint32_t frequency) {
    if (!HAL_timer_initialized(timer_num)) {
        ...
        case MF_TIMER_TEMP: // TEMP TIMER - any available 16bit timer
            ...
            break;
        case MF_TIMER_LUCAS: // Lucas TIMER - any available 16bit timer
            timer_instance[timer_num] = new HardwareTimer(LUCAS_TIMER_DEV);
            // The prescale factor is computed automatically for HERTZ_FORMAT
            timer_instance[timer_num]->setOverflow(frequency, HERTZ_FORMAT);
            break;
        }
        ...
        case MF_TIMER_STEP:
            ...
            break;
        case MF_TIMER_LUCAS:
            timer_instance[timer_num]->setInterruptPriority(2, 0);
            break;
        case MF_TIMER_TEMP:
...
static constexpr int get_timer_num_from_base_address(uintptr_t base_address) {
    for (const auto& timer : stm32_timer_map)
        if (timer.base_address == base_address)
            return timer.timer_number;
    return 0;
}
