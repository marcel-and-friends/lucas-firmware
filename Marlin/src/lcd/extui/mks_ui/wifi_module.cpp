...
static void wifi_usart_dma_init() {
    ...
    dma_setup_transfer(DMA1, DMA_CH5, &USART1_BASE->DR, dma_bit_size, (volatile void*)WIFISERIAL.usart_device->rb->buf, dma_bit_size, flags); // Transmit buffer DMA
    ...
    if (type == WIFI_PARA_SET) {
        ...
        if (len > 0) {
            ...
            if (buf_to_wifi[index_to_wifi + 3] == '\n') {
                // mask "wait" "busy" "X:"
                if (((buf_to_wifi[4] == 'w') && (buf_to_wifi[5] == 'a') && (buf_to_wifi[6] == 'i') && (buf_to_wifi[7] == 't')) || ((buf_to_wifi[4] == 'b') && (buf_to_wifi[5] == 'u') && (buf_to_wifi[6] == 's') && (buf_to_wifi[7] == 'y')) || ((buf_to_wifi[4] == 'X') && (buf_to_wifi[5] == ':'))) {
                    ZERO(buf_to_wifi);
                    index_to_wifi = 0;
                    return 0;
                }
...
static void net_msg_handle(uint8_t* msg, uint16_t msgLen) {
    ...
    if (uiCfg.configWifi) {
        if ((wifiPara.mode != gCfgItems.wifi_mode_sel) || (strncmp(wifiPara.ap_name, (const char*)uiCfg.wifi_name, 32) != 0) || (strncmp(wifiPara.keyCode, (const char*)uiCfg.wifi_key, 64) != 0)) {
            package_to_wifi(WIFI_PARA_SET, (uint8_t*)0, 0);
        } else
            uiCfg.configWifi = false;
    }
    if (cfg_cloud_flag == 1) {
        if (((cloud_para.state >> 4) != (char)gCfgItems.cloud_enable) || (strncmp(cloud_para.hostUrl, (const char*)uiCfg.cloud_hostUrl, 96) != 0) || (cloud_para.port != uiCfg.cloud_port))
...
void esp_data_parser(char* cmdRxBuf, int len) {
    ...
    while (leftLen > 0 || loop_again) {
        ...
        if (esp_msg_index != 0) {
            ...
            head_pos = charAtArray((const uint8_t*)&cmdRxBuf[len - leftLen], leftLen, ESP_PROTOC_HEAD);
            if (head_pos == -1)
                return;
        ...
        if (esp_frame.type != ESP_TYPE_NET && esp_frame.type != ESP_TYPE_GCODE && esp_frame.type != ESP_TYPE_FILE_FIRST && esp_frame.type != ESP_TYPE_FILE_FRAGMENT && esp_frame.type != ESP_TYPE_WIFI_LIST) {
            ZERO(esp_msg_buf);
            esp_msg_index = 0;
            return;
        }
...
void wifi_rcv_handle() {
    ...
    #else
        #ifndef __STM32F1__
        if (wifiDmaRcvFifo.receiveEspData)
            storeRcvData(WIFISERIAL.wifiRxBuf, UART_RX_BUFFER_SIZE);
        #endif
    ...
    if (espGcodeFifo.wait_tick > 5) {
        while (!queue.ring_buffer.full() && (espGcodeFifo.r != espGcodeFifo.w)) {
            ...
            if (wifi_char == '\n' || wifi_char == '\r') {
                ...
                if (wifi_char == ';')
                    wifi_comment_mode = true;
                if (!wifi_comment_mode)
                    wifi_line_buffer[wifi_read_count++] = wifi_char;
            }
        }
