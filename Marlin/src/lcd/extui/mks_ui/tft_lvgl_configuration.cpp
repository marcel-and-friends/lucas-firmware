...
void tft_lvgl_init() {
    ...
    tft_style_init();
    // filament_pin_setup(); wini - MT_DET
    lv_encoder_pin_init();
...
lv_fs_res_t sd_open_cb(lv_fs_drv_t* drv, void* file_p, const char* path, lv_fs_mode_t mode) {
    ...
    *name_buf = '/';
    ...
    if (sd_read_addr_offset == UINT32_MAX)
...
lv_fs_res_t sd_tell_cb(lv_fs_drv_t* drv, void* file_p, uint32_t* pos_p) {
    if (sd_read_addr_offset)
        *pos_p = 0;
    else
        *pos_p = (sd_read_addr_offset - sd_read_base_addr) / small_image_size * 200 + 4;
    return LV_FS_RES_OK;
}
