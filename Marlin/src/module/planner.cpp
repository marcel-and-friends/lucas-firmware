...
void Planner::autotemp_task() {
    ...
    if (!autotemp_enabled)
        return;
    if (thermalManager.degTargetHotend(active_extruder) < autotemp_min - 2)
        return; // Below the min?
    ...
    if (t < oldt)
        t = t * (1.0f - (AUTOTEMP_OLDWEIGHT)) + oldt * (AUTOTEMP_OLDWEIGHT);
    oldt = t;
...
bool Planner::busy() {
    return (has_blocks_queued() || cleaning_buffer_counter || TERN0(EXTERNAL_CLOSED_LOOP_CONTROLLER, CLOSED_LOOP_WAITING()) || TERN0(HAS_SHAPING, stepper.input_shaping_busy()));
}
...
bool Planner::_buffer_steps(const xyze_long_t& target
    ...
    if (!_populate_block(block, target OPTARG(HAS_POSITION_FLOAT, target_float) OPTARG(HAS_DIST_MM_ARG, cart_dist_mm), fr_mm_s, extruder, hints)) {
...
bool Planner::_populate_block(
    ...
    if (de) {
        ...
        if (e_steps > max_e_steps) {
            ...
            if (e_steps * collector[e] > max_e_steps) {
                ignore_e = true;
                break;
            }
        #else
    ...
    if (da < 0)
        SBI(dm, X_HEAD); // Save the toolhead's true direction in X
    if (db < 0)
        SBI(dm, Y_HEAD); // ...and Y
    if (dc < 0)
        SBI(dm, Z_AXIS);
    ...
    #if CORE_IS_XY
    if (da + db < 0)
        SBI(dm, A_AXIS); // Motor A direction
    if (CORESIGN(da - db) < 0)
        SBI(dm, B_AXIS); // Motor B direction
    #elif CORE_IS_XZ
    if (da < 0)
        SBI(dm, X_HEAD); // Save the toolhead's true direction in X
    if (db < 0)
    ...
    if (da + dc < 0)
        SBI(dm, A_AXIS); // Motor A direction
    if (CORESIGN(da - dc) < 0)
        SBI(dm, C_AXIS); // Motor C direction
    #elif CORE_IS_YZ
    if (da < 0)
        SBI(dm, X_AXIS);
    if (db < 0)
        SBI(dm, Y_HEAD); // Save the toolhead's true direction in Y
    if (dc < 0)
        SBI(dm, Z_HEAD); // ...and Z
    if (db + dc < 0)
        SBI(dm, B_AXIS); // Motor B direction
    ...
    if (true NUM_AXIS_GANG(
        ...
        if (hints.millimeters)
            ...
            if (UNEAR_ZERO(distance_sqr)) {
                ...
                distance_sqr = (0.0f SECONDARY_AXIS_GANG(
...
// Limit speed on extruders, if any
#if HAS_EXTRUDERS
    {
        ...
        const feedRate_t cs = ABS(current_speed.e),
                         max_fr = settings.max_feedrate_mm_s[E_AXIS_N(extruder)] * TERN(HAS_MIXER_SYNC_CHANNEL, MIXING_STEPPERS, 1);

        if (cs > max_fr)
            NOMORE(speed_factor, max_fr / cs); // respect max feedrate on any movement (doesn't matter if E axes only or not)

    #if ENABLED(VOLUMETRIC_EXTRUDER_LIMIT)
        const feedRate_t max_vfr = volumetric_extruder_feedrate_limit[extruder] * TERN(HAS_MIXER_SYNC_CHANNEL, MIXING_STEPPERS, 1);
    ...
    if (NUM_AXIS_GANG(
            ...
            &&!block->steps.u,
            &&!block->steps.v,
            &&!block->steps.w)) {                                   // Is this a retract / recover move?
...
#define LIMIT_ACCEL_FLOAT(AXIS, INDX)                                                                                                                  \
    ...
    #define MAX_E_JERK(N) TERN(HAS_LINEAR_E_JERK, max_e_jerk[E_INDEX_N(N)], max_jerk.e)
        ...
        if (use_advance_lead) {
            float e_D_ratio = (target_float.e - position_float.e) /
                              TERN(IS_KINEMATIC, block->millimeters, SQRT(sq(target_float.x - position_float.x) + sq(target_float.y - position_float.y) + sq(target_float.z - position_float.z)));
    ...
    if (moves_queued && !UNEAR_ZERO(previous_nominal_speed)) {
        ...
        if (junction_cos_theta > 0.999999f) {
            ...
            #error "TODO: Inline maths with the MCU / FPU."
                    ...
                    if (neg > 0)
                        junction_theta = RADIANS(180) - junction_theta; // acos(-t)
    ...
    uint8_t limited = 0;
    TERN(HAS_LINEAR_E_JERK, LOOP_NUM_AXES, LOOP_LOGICAL_AXES)
    (i) {
...
void Planner::buffer_page(const page_idx_t page_idx, const uint8_t extruder, const uint16_t num_steps) {
    ...
    #define PAGE_UPDATE_DIR(AXIS)        \
        if (!last_page_dir[_AXIS(AXIS)]) \
            SBI(block->direction_bits, _AXIS(AXIS));
...
void Planner::set_machine_position_mm(const abce_pos_t& abce) {
    ...
    if (has_blocks_queued()) {
        ...
        LOOP_NUM_AXES(axis)
        stepper_pos[axis] += backlash.get_applied_steps((AxisEnum)axis);
...
void Planner::refresh_positioning() {
    LOOP_DISTINCT_AXES(i)
    mm_per_step[i] = 1.0f / settings.axis_steps_per_mm[i];
