...
#if ANY(TEMP_SENSOR_0_IS_THERMISTOR, TEMP_SENSOR_1_IS_THERMISTOR, TEMP_SENSOR_2_IS_THERMISTOR, TEMP_SENSOR_3_IS_THERMISTOR, TEMP_SENSOR_4_IS_THERMISTOR, TEMP_SENSOR_5_IS_THERMISTOR, TEMP_SENSOR_6_IS_THERMISTOR, TEMP_SENSOR_7_IS_THERMISTOR)
...
void Temperature::set_fan_speed(uint8_t fan, uint16_t speed) {
    ...
    if (fan != active_extruder) {
        if (fan < EXTRUDERS)
            singlenozzle_fan_speed[fan] = speed;
        return;
    }
    #endif
    ...
    if (fan == 0)
        fan_speed[REDUNDANT_PART_COOLING_FAN] = speed;
    #endif
...
void Temperature::PID_autotune(const celsius_t target, const heater_id_t heater_id, const int8_t ncycles, const bool set_result /*=false*/) {
    ...
    // Timeout after MAX_CYCLE_TIME_PID_AUTOTUNE minutes since the last undershoot/overshoot cycle
        ...
        #else
                HOTEND_LOOP()
                thermalManager.temp_hotend[e].pid.set(in_pid);
        #endif
                updatePID();
    #endif
                UNUSED(tool);
...
void Temperature::MPC_autotune() {
    ...
    for (;;) { // Can be interrupted with M108
        if (!housekeeping(ms, current_temp, next_report_ms))
            return;
    ...
    for (;;) { // Can be interrupted with M108
        ...
        if (ELAPSED(ms, next_test_ms)) {
            ...
            if (current_temp >= 100.0f) {
                ...
                if (sample_count == 0)
                    t1_time = float(ms - heat_start_time) / 1000.0f;
                temp_samples[sample_count++] = current_temp;
            }

            if (current_temp >= 200.0f)
                break;
    ...
    #endif
            else if (ELAPSED(ms, test_end_ms))
                break;
...
#if HAS_COOLER
    case H_COOLER:
        return temp_cooler.soft_pwm_amount;
#endif
    default:
...
void Temperature::_temp_error(const heater_id_t heater_id, FSTR_P const serial_msg, FSTR_P const lcd_msg) {
    ...
    if (IsRunning() && TERN1(BOGUS_TEMPERATURE_GRACE_PERIOD, killed == 2)) {
        ...
        switch (real_heater_id) {
            OPTCODE(HAS_TEMP_COOLER, case H_COOLER
                    : SERIAL_ECHOPGM(STR_COOLER);
                    break)
            OPTCODE(HAS_TEMP_PROBE, case H_PROBE
                    : SERIAL_ECHOPGM(STR_PROBE);
                    break)
            OPTCODE(HAS_TEMP_BOARD, case H_BOARD
                    : SERIAL_ECHOPGM(STR_MOTHERBOARD);
                    break)
            OPTCODE(HAS_TEMP_CHAMBER, case H_CHAMBER
                    : SERIAL_ECHOPGM(STR_HEATER_CHAMBER);
                    break)
            OPTCODE(HAS_TEMP_BED, case H_BED
...
void Temperature::manage_heated_bed(const millis_t& ms) {
    ...
    do {
        ...
        if (PENDING(ms, next_bed_check_ms) && TERN1(PAUSE_CHANGE_REQD, paused_for_probing == last_pause_state))
            break;
...
void Temperature::manage_heated_chamber(const millis_t& ms) {
    ...
    if (ELAPSED(ms, next_chamber_check_ms)) {
        ...
        #if ENABLED(CHAMBER_VENT)
                if (!flag_chamber_off)
                    servo[CHAMBER_VENT_SERVO_NR].move(temp_chamber.is_below_target() ? 0 : 90);
        #endif
            } else {
        ...
        #if ENABLED(CHAMBER_VENT)
                if (!flag_chamber_off)
                    servo[CHAMBER_VENT_SERVO_NR].move(0);
        #endif
            }
        } else {
            temp_chamber.soft_pwm_amount = 0;
...
void Temperature::manage_cooler(const millis_t& ms) {
    ...
    if (degCooler() > COOLER_MAXTEMP)
        max_temp_error(H_COOLER);
    #endif
    ...
    if (cooler.enabled) {
        ...
        if (temp_cooler.target == 0)
            temp_cooler.target = COOLER_MIN_TARGET;
...
void Temperature::task() {
    ...
    if (degHotend(2) > _MIN(HEATER_2_MAXTEMP, TEMP_SENSOR_2_MAX_TC_TMAX - 1.0))
        max_temp_error(H_E2);
    if (degHotend(2) < _MAX(HEATER_2_MINTEMP, TEMP_SENSOR_2_MAX_TC_TMIN + .01))
        min_temp_error(H_E2);
    #endif
    #if TEMP_SENSOR_IS_MAX_TC(REDUNDANT)
    if (degRedundant() > TEMP_SENSOR_REDUNDANT_MAX_TC_TMAX - 1.0)
        max_temp_error(H_REDUNDANT);
    if (degRedundant() < TEMP_SENSOR_REDUNDANT_MAX_TC_TMIN + .01)
        min_temp_error(H_REDUNDANT);
    #endif
#else
                                                ...
                                                TERN_(TEMP_SENSOR_COOLER_IS_CUSTOM, t_index == CTI_COOLER ? F("COOLER") :)
                                                    TERN_(TEMP_SENSOR_PROBE_IS_CUSTOM, t_index == CTI_PROBE ? F("PROBE") :)
                                                        TERN_(TEMP_SENSOR_BOARD_IS_CUSTOM, t_index == CTI_BOARD ? F("BOARD") :)
                                                            TERN_(TEMP_SENSOR_REDUNDANT_IS_CUSTOM, t_index == CTI_REDUNDANT ? F("REDUNDANT") :) nullptr);
    SERIAL_EOL();
}
...
celsius_float_t Temperature::user_thermistor_to_deg_c(const uint8_t t_index, const raw_adc_t raw) {
    ...
    if (t.pre_calc) { // pre-calculate some variables
        ...
        t.sh_alpha = RECIPROCAL(THERMISTOR_RESISTANCE_NOMINAL_C - (THERMISTOR_ABS_ZERO_C)) - (t.beta_recip * t.res_25_log) - (t.sh_c_coeff * cu(t.res_25_log));
    }
...
#define TP_CMP(S, A, B) (TEMPDIR(S) < 0 ? ((A) < (B)) : ((A) > (B)))
    ...
    if (TP_CMP(CHAMBER, temp_chamber.getraw(), maxtemp_raw_CHAMBER))
        max_temp_error(H_CHAMBER);
    ...
    if (cutter.unitPower > 0 && TP_CMP(COOLER, temp_cooler.getraw(), maxtemp_raw_COOLER))
        max_temp_error(H_COOLER);
    if (TP_CMP(COOLER, mintemp_raw_COOLER, temp_cooler.getraw()))
        min_temp_error(H_COOLER);
#endif

#if BOTH(HAS_TEMP_BOARD, THERMAL_PROTECTION_BOARD)
    if (TP_CMP(BOARD, temp_board.getraw(), maxtemp_raw_BOARD))
        max_temp_error(H_BOARD);
    if (TP_CMP(BOARD, mintemp_raw_BOARD, temp_board.getraw()))
        min_temp_error(H_BOARD);
#endif
#undef TP_CMP
...
#if HAS_COOLER
    while (analog_to_celsius_cooler(mintemp_raw_COOLER) > COOLER_MINTEMP)
        mintemp_raw_COOLER += TEMPDIR(COOLER) * (OVERSAMPLENR);
    while (analog_to_celsius_cooler(maxtemp_raw_COOLER) < COOLER_MAXTEMP)
        maxtemp_raw_COOLER -= TEMPDIR(COOLER) * (OVERSAMPLENR);
#endif

#if BOTH(HAS_TEMP_BOARD, THERMAL_PROTECTION_BOARD)
    while (analog_to_celsius_board(mintemp_raw_BOARD) < BOARD_MINTEMP)
        mintemp_raw_BOARD += TEMPDIR(BOARD) * (OVERSAMPLENR);
    while (analog_to_celsius_board(maxtemp_raw_BOARD) > BOARD_MAXTEMP)
        maxtemp_raw_BOARD -= TEMPDIR(BOARD) * (OVERSAMPLENR);
#endif
...
raw_adc_t Temperature::read_max_tc(TERN_(HAS_MULTI_MAX_TC, const uint8_t hindex /*=0*/)) {
    ...
    static raw_adc_t max_tc_temp_previous[MAX_TC_COUNT] = { 0 };
        ...
        #if MAX_TC_COUNT > 2
            #define THERMO_SEL(A, B, C) (hindex > 1 ? (C) : hindex == 1 ? (B) \
                                                                        : (A))
            #define MAXTC_CS_WRITE(V)            \
                do {                             \
                    switch (hindex) {            \
                    case 1:                      \
                        WRITE(TEMP_1_CS_PIN, V); \
                        break;                   \
                    case 2:                      \
                        WRITE(TEMP_2_CS_PIN, V); \
                        break;                   \
                    default:                     \
                        WRITE(TEMP_0_CS_PIN, V); \
                    }                            \
                } while (0)
        #elif MAX_TC_COUNT > 1
    ...
    for (uint8_t i = sizeof(max_tc_temp); i--;) {
        ...
        if (i > 0)
            max_tc_temp <<= 8; // shift left if not the last byte
    }
    ...
    if (max_tc_temp & 0x00002000)
        max_tc_temp |= 0xFFFFC000;
    #endif
...
#define WRITE_FAN(n, v) WRITE(FAN##n##_PIN, (v) ^ FAN_INVERTING)
    ...
    #define _PWM_LOW(N, S)                \
        ...
        #endif
        #if HAS_FAN3
        if (soft_pwm_count_fan[3] <= pwm_count_tmp)
            WRITE_FAN(3, LOW);
        #endif
        #if HAS_FAN4
        if (soft_pwm_count_fan[4] <= pwm_count_tmp)
            WRITE_FAN(4, LOW);
        #endif
        #if HAS_FAN5
        if (soft_pwm_count_fan[5] <= pwm_count_tmp)
            WRITE_FAN(5, LOW);
        #endif
        #if HAS_FAN6
        if (soft_pwm_count_fan[6] <= pwm_count_tmp)
            WRITE_FAN(6, LOW);
        #endif
        #if HAS_FAN7
        if (soft_pwm_count_fan[7] <= pwm_count_tmp)
            WRITE_FAN(7, LOW);
        #endif
        #if ENABLED(USE_CONTROLLER_FAN)
...
#pragma GCC diagnostic pop
    ...
    case PrepareTemp_CHAMBER:
        hal.adc_start(TEMP_CHAMBER_PIN);
        break;
    ...
    case PrepareTemp_COOLER:
        hal.adc_start(TEMP_COOLER_PIN);
        break;
    case MeasureTemp_COOLER:
        ACCUMULATE_ADC(temp_cooler);
        break;
#endif

#if HAS_TEMP_ADC_PROBE
    case PrepareTemp_PROBE:
        hal.adc_start(TEMP_PROBE_PIN);
        break;
    ...
    case PrepareTemp_REDUNDANT:
        hal.adc_start(TEMP_REDUNDANT_PIN);
        break;
    case MeasureTemp_REDUNDANT:
        ACCUMULATE_ADC(temp_redundant);
        break;
#endif

#if HAS_TEMP_ADC_1
    case PrepareTemp_1:
        hal.adc_start(TEMP_1_PIN);
        break;
    case MeasureTemp_1:
    ...
    case Prepare_FILWIDTH:
        hal.adc_start(FILWIDTH_PIN);
        break;
    case Measure_FILWIDTH:
        if (!hal.adc_ready())
            next_sensor_state = adc_sensor_state; // Redo this state
        else
            filwidth.accumulate(hal.adc_value());
        break;
#endif
    ...
    case Measure_POWER_MONITOR_CURRENT:
        if (!hal.adc_ready())
            ...
            power_monitor.add_current_sample(hal.adc_value());
        break;
#endif
    ...
    case Measure_POWER_MONITOR_VOLTAGE:
        if (!hal.adc_ready())
            ...
            power_monitor.add_voltage_sample(hal.adc_value());
        break;
#endif

#if HAS_JOY_ADC_X
    case PrepareJoy_X:
        hal.adc_start(JOY_X_PIN);
        break;
    case MeasureJoy_X:
 ...
 */
static void print_heater_state(const heater_id_t e, const_celsius_float_t c, const_celsius_float_t t OPTARG(SHOW_TEMP_ADC_VALUES, const float r)) {
    char k;
    switch (e) {
    default:
    #if HAS_TEMP_HOTEND
...
void Temperature::print_heater_states(const int8_t target_extruder
    ...
    #if HAS_TEMP_REDUNDANT
    if (include_r)
        print_heater_state(H_REDUNDANT, degRedundant(), degRedundantTarget() OPTARG(SHOW_TEMP_ADC_VALUES, rawRedundantTemp()));
    #endif
    #if HAS_MULTI_HOTEND
    HOTEND_LOOP()
    print_heater_state((heater_id_t)e, degHotend(e), degTargetHotend(e) OPTARG(SHOW_TEMP_ADC_VALUES, rawHotendTemp(e)));
    #endif
...
bool Temperature::wait_for_bed(const bool no_wait_for_cooling /*=true*/
    ...
    do {
        ...
        } else if (TEMP_BED_HYSTERESIS && temp_diff > TEMP_BED_HYSTERESIS) {
            // Restart the timer whenever the temperature falls outside the hysteresis.
            residency_start_ms = now;
        }
    ...
    do {
        ...
        if (wants_to_cool) {
            ...
            if (!next_cooling_check_ms || ELAPSED(now, next_cooling_check_ms)) {
                if (previous_temp - current_temp < float(MIN_COOLING_SLOPE_DEG_COOLER))
                    break;
