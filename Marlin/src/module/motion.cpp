...
void report_current_grblstate_moving() {
    SERIAL_ECHOLNPGM("S_XYZ:", int(M_State_grbl));
}
...
bool position_is_reachable(const_float_t rx, const_float_t ry, const float inset /*=0*/) {
    ...
    can_reach = (a < polargraph_max_belt_len + 1 && b < polargraph_max_belt_len + 1);
...
bool position_is_reachable(const_float_t rx, const_float_t ry) {
    if (!COORDINATE_OKAY(ry, Y_MIN_POS - fslop, Y_MAX_POS + fslop))
        return false;
...
void do_blocking_move_to(const xy_pos_t& raw, const_feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST(raw.x, raw.y, current_position.z, current_position.i, current_position.j, current_position.k, current_position.u, current_position.v, current_position.w), fr_mm_s);
}
...
void do_blocking_move_to_x(const_float_t rx, const_feedRate_t fr_mm_s /*=0.0*/) {
    do_blocking_move_to(
        NUM_AXIS_LIST(rx, current_position.y, current_position.z, current_position.i, current_position.j, current_position.k, current_position.u, current_position.v, current_position.w),
        fr_mm_s);
}
...
void do_blocking_move_to_y(const_float_t ry, const_feedRate_t fr_mm_s /*=0.0*/) {
    do_blocking_move_to(
        NUM_AXIS_LIST(current_position.x, ry, current_position.z, current_position.i, current_position.j, current_position.k, current_position.u, current_position.v, current_position.w),
        fr_mm_s);
}
#endif
...
void do_blocking_move_to_xy(const_float_t rx, const_float_t ry, const_feedRate_t fr_mm_s /*=0.0*/) {
    do_blocking_move_to(
        NUM_AXIS_LIST(rx, ry, current_position.z, current_position.i, current_position.j, current_position.k, current_position.u, current_position.v, current_position.w),
        fr_mm_s);
}
...
void do_blocking_move_to_xy_z(const xy_pos_t& raw, const_float_t z, const_feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(
        NUM_AXIS_LIST(raw.x, raw.y, z, current_position.i, current_position.j, current_position.k, current_position.u, current_position.v, current_position.w),
        fr_mm_s);
}
...
inline bool dual_x_carriage_unpark() {
    if (active_extruder_parked) {
        ...
        case DXC_AUTO_PARK_MODE: {
            ...
            xyze_pos_t raised = raised_parked_position;
            raised.e = current_position.e;
            if (planner.buffer_line(raised, fr_zfast)) {
                //  2. Move to the current native XY and raised Z. Presumably this is a null move.
                xyze_pos_t curpos = current_position;
                curpos.z = raised_parked_position.z;
        ...
        case DXC_DUPLICATION_MODE:
            if (active_extruder == 0) {
                ...
                if (DEBUGGING(LEVELING))
                    DEBUG_ECHOLNPGM("Set planner X", inactive_extruder_x, " ... Line to X", new_pos.x);
...
void prepare_line_to_destination() {
    ...
    if (TERN0(DUAL_X_CARRIAGE, dual_x_carriage_unpark()))
        return;
...
main_axes_bits_t axes_should_home(main_axes_bits_t axis_bits /*=main_axes_mask*/) {
    ...
    NUM_AXIS_CODE(
        ...
        set_should(axis_bits, I_AXIS),
        set_should(axis_bits, J_AXIS),
        set_should(axis_bits, K_AXIS),
        set_should(axis_bits, U_AXIS),
        set_should(axis_bits, V_AXIS),
        set_should(axis_bits, W_AXIS));
    return axis_bits;
}
...
feedRate_t get_homing_bump_feedrate(const AxisEnum axis) {
    ...
    if (axis == Z_AXIS)
        return MMM_TO_MMS(Z_PROBE_FEEDRATE_SLOW);
    #endif
...
sensorless_t start_sensorless_homing_per_axis(const AxisEnum axis) {
    ...
    case Z_AXIS:
        ...
        break;
            ...
            #if HAS_I_AXIS
    case I_AXIS:
        if (ENABLED(I_SPI_SENSORLESS))
            endstops.tmc_spi_homing.i = true;
        break;
            #endif
            #if HAS_J_AXIS
    case J_AXIS:
        if (ENABLED(J_SPI_SENSORLESS))
            endstops.tmc_spi_homing.j = true;
        break;
            #endif
            #if HAS_K_AXIS
    case K_AXIS:
        if (ENABLED(K_SPI_SENSORLESS))
            endstops.tmc_spi_homing.k = true;
        break;
            #endif
            #if HAS_U_AXIS
    case U_AXIS:
        if (ENABLED(U_SPI_SENSORLESS))
            endstops.tmc_spi_homing.u = true;
        break;
            #endif
            #if HAS_V_AXIS
    case V_AXIS:
        if (ENABLED(V_SPI_SENSORLESS))
            endstops.tmc_spi_homing.v = true;
        break;
            #endif
            #if HAS_W_AXIS
    case W_AXIS:
        if (ENABLED(W_SPI_SENSORLESS))
            endstops.tmc_spi_homing.w = true;
        break;
            #endif
    default:
        break;
...
void end_sensorless_homing_per_axis(const AxisEnum axis, sensorless_t enable_stealth) {
    ...
    case I_AXIS:
        if (ENABLED(I_SPI_SENSORLESS))
            endstops.tmc_spi_homing.i = false;
        break;
            #endif
            #if HAS_J_AXIS
    case J_AXIS:
        if (ENABLED(J_SPI_SENSORLESS))
            endstops.tmc_spi_homing.j = false;
        break;
            #endif
            #if HAS_K_AXIS
    case K_AXIS:
        if (ENABLED(K_SPI_SENSORLESS))
            endstops.tmc_spi_homing.k = false;
        break;
            #endif
            #if HAS_U_AXIS
    case U_AXIS:
        if (ENABLED(U_SPI_SENSORLESS))
            endstops.tmc_spi_homing.u = false;
        break;
            #endif
            #if HAS_V_AXIS
    case V_AXIS:
        if (ENABLED(V_SPI_SENSORLESS))
            endstops.tmc_spi_homing.v = false;
        break;
            #endif
            #if HAS_W_AXIS
    case W_AXIS:
        if (ENABLED(W_SPI_SENSORLESS))
            endstops.tmc_spi_homing.w = false;
        break;
            #endif
    default:
        break;
...
void set_axis_never_homed(const AxisEnum axis) {
    if (DEBUGGING(LEVELING))
        DEBUG_ECHOLNPGM(">>> set_axis_never_homed(", AS_CHAR(AXIS_CHAR(axis)), ")");
...
void backout_to_tmc_homing_phase(const AxisEnum axis) {
    ...
    if (ABS(phaseDelta) * planner.mm_per_step[axis] / phasePerUStep < 0.05f)
        SERIAL_ECHOLNPGM("Selected home phase ", home_phase[axis], " too close to endstop trigger phase ", phaseCurrent, ". Pick a different phase for ", AS_CHAR(AXIS_CHAR(axis)));
...
void homeaxis(const AxisEnum axis) {
    ...
    #else
        #define _CAN_HOME(A) (axis == _AXIS(A) && (ENABLED(A##_SPI_SENSORLESS) || TERN0(HAS_Z_AXIS, TERN0(HOMING_Z_WITH_PROBE, _AXIS(A) == Z_AXIS)) || TERN0(A##_HOME_TO_MIN, A##_MIN_PIN > -1) || TERN0(A##_HOME_TO_MAX, A##_MAX_PIN > -1)))
    ...
    if (DEBUGGING(LEVELING))
        DEBUG_ECHOLNPGM(">>> homeaxis(", AS_CHAR(AXIS_CHAR(axis)), ")");
    ...
    if (axis == Z_AXIS) {
        if (TERN0(BLTOUCH, bltouch.deploy()))
            return; // BLTouch was deployed above, but get the alarm state.
    ...
    if ((TERN0(X_SENSORLESS, axis == X_AXIS) || TERN0(Y_SENSORLESS, axis == Y_AXIS) || TERN0(Z_SENSORLESS, axis == Z_AXIS) || TERN0(I_SENSORLESS, axis == I_AXIS) || TERN0(J_SENSORLESS, axis == J_AXIS) || TERN0(K_SENSORLESS, axis == K_AXIS)) && backoff[axis]) {
        ...
        if (DEBUGGING(LEVELING))
            DEBUG_ECHOLNPGM("Sensorless backoff: ", backoff_length, "mm");
    ...
    const float bump = axis_home_dir * (use_probe_bump ? _MAX(TERN0(HOMING_Z_WITH_PROBE, Z_CLEARANCE_BETWEEN_PROBES), home_bump_mm(axis)) : home_bump_mm(axis));
    ...
    if (DEBUGGING(LEVELING))
        DEBUG_ECHOLNPGM("Home Fast: ", move_length, "mm");
    do_homing_move(axis, move_length, 0.0, !use_probe_bump);

    #if BOTH(HOMING_Z_WITH_PROBE, BLTOUCH)
    if (axis == Z_AXIS && !bltouch.high_speed_mode)
    ...
    if (bump) {
        ...
        if (DEBUGGING(LEVELING))
            DEBUG_ECHOLNPGM("Move Away: ", -bump, "mm");
        ...
        if (DEBUGGING(LEVELING))
            DEBUG_ECHOLNPGM("Re-bump: ", rebump, "mm");
    ...
    if (adjDistance * (Z_HOME_DIR) < 0 && ABS(adjDistance) > minDistance) { // away from endstop, more than min distance
        if (DEBUGGING(LEVELING))
            DEBUG_ECHOLNPGM("adjDistance:", adjDistance);
 ...
 */
    ...
    if (axis == Z_AXIS) {
        ...
        if (DEBUGGING(LEVELING))
            DEBUG_ECHOLNPGM("*** Z HOMED WITH PROBE (Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN) ***\n> probe.offset.z = ", probe.offset.z);
...
void update_workspace_offset(const AxisEnum axis) {
    ...
    if (DEBUGGING(LEVELING))
        DEBUG_ECHOLNPGM("Axis ", AS_CHAR(AXIS_CHAR(axis)), " home_offset = ", home_offset[axis], " position_shift = ", position_shift[axis]);
}
#endif
