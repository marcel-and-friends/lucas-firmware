...
inline float pgm_read_any(const float* p) {
    return TERN(__IMXRT1062__, *p, pgm_read_float(p));
}
inline int8_t pgm_read_any(const int8_t* p) {
    return TERN(__IMXRT1062__, *p, pgm_read_byte(p));
}
...
//     static const XYZval<float> base_max_pos_P = { X_MAX_POS, 120, 0 };
...
struct PositionReport {
    static void report() { report_current_position_projected(); }
};
...
inline void do_blocking_move_to_xy(const xyz_pos_t& raw, const_feedRate_t fr_mm_s = 0.0f) {
    do_blocking_move_to_xy(xy_pos_t(raw), fr_mm_s);
}
inline void do_blocking_move_to_xy(const xyze_pos_t& raw, const_feedRate_t fr_mm_s = 0.0f) {
    do_blocking_move_to_xy(xy_pos_t(raw), fr_mm_s);
}
#endif
...
inline void do_blocking_move_to_xy_z(const xyz_pos_t& raw, const_float_t z, const_feedRate_t fr_mm_s = 0.0f) {
    do_blocking_move_to_xy_z(xy_pos_t(raw), z, fr_mm_s);
}
inline void do_blocking_move_to_xy_z(const xyze_pos_t& raw, const_float_t z, const_feedRate_t fr_mm_s = 0.0f) {
    do_blocking_move_to_xy_z(xy_pos_t(raw), z, fr_mm_s);
}
#endif
...
inline main_axes_bits_t axes_should_home(main_axes_bits_t = main_axes_mask) {
    return 0;
}
inline bool homing_needed_error(main_axes_bits_t = main_axes_mask) {
    return false;
}
inline void set_axis_unhomed(const AxisEnum axis) {
}
...
inline bool axis_was_homed(const AxisEnum axis) {
    return TEST(axes_homed, axis);
}
inline bool axis_is_trusted(const AxisEnum axis) {
    return TEST(axes_trusted, axis);
}
inline bool axis_should_home(const AxisEnum axis) {
    return (axes_should_home() & _BV(axis)) != 0;
}
inline bool no_axes_homed() {
    return !axes_homed;
}
inline bool all_axes_homed() {
    return main_axes_mask == (axes_homed & main_axes_mask);
}
inline bool homing_needed() {
    return !all_axes_homed();
}
inline bool all_axes_trusted() {
    return main_axes_mask == (axes_trusted & main_axes_mask);
}
...
inline void toLogical(xy_pos_t& raw) {
    raw += _WS;
}
inline void toLogical(xyz_pos_t& raw) {
    raw += _WS;
}
inline void toLogical(xyze_pos_t& raw) {
    raw += _WS;
}
inline void toNative(xy_pos_t& raw) {
    raw -= _WS;
}
inline void toNative(xyz_pos_t& raw) {
    raw -= _WS;
}
inline void toNative(xyze_pos_t& raw) {
    raw -= _WS;
}
#else
...
inline bool idex_is_duplicating() {
...
inline void set_duplication_enabled(const bool dupe) {
