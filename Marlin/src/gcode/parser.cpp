...
char* GCodeParser::unescape_string(char*& src) {
    ...
    for (;;) {
        ...
        case '\\':
            c = *src++;
            break; // Get the escaped char
        case '"':
            c = '\0';
            break; // Convert bare quote to nul
        }
        if (!(*dst++ = c))
            break; // Copy and break on nul
    }
    return out;
}
...
// Allow for decimal point in command
...
#if ENABLED(GCODE_MOTION_MODES)
            if (letter == 'G' && (codenum <= TERN(ARC_SUPPORT, 3, 1) || TERN0(BEZIER_CURVE_SUPPORT, codenum == 5) || TERN0(G38_PROBE_TARGET, codenum == 38))) {
    ...
    case 'I' ... 'J':
    case 'P':
        if (TERN1(BEZIER_CURVE_SUPPORT, motion_mode_codenum != 5) && TERN1(ARC_P_CIRCLES, !WITHIN(motion_mode_codenum, 2, 3)))
            return;
    #endif
    ...
    case 'R':
        if (!WITHIN(motion_mode_codenum, 2, 3))
            return;
    #endif

        LOGICAL_AXIS_GANG(case 'E'
                          :, case 'X'
                          :, case 'Y'
                          :, case 'Z'
 ...
 */
    ...
    while (const char param = uppercase(*p++)) { // Get the next parameter. A NUL ends the loop
        ...
        if (param == '!' && is_command('M', 32)) {
            ...
            if (lb)
                *lb = '\0'; // Safe to mark the end of the filename
            return;
        }
