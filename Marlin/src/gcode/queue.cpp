...
void GCodeQueue::enqueue_one_now(FSTR_P const fcmd) {
    while (!enqueue_one(fcmd))
        idle();
}
...
void GCodeQueue::gcode_line_error(FSTR_P const ferr, const serial_index_t serial_ind) {
    ...
    while (read_serial(serial_ind) != -1) { /* nada */
...
// send "wait" to indicate Marlin is still waiting.
    ...
    for (bool hadData = true; hadData;) {
        ...
        LOOP_L_N(p, NUM_SERIAL) {
            ...
            // é possível um delimiter de mensagem especial (json) passar despercebido pela 'serial::hooks'
            // pois entre aquela função retornar e essa aqui ser chamada uma nova mensagem pode ter sido enviada pelo app
            // FIXME: usar uma lista de delimiteres
...
void GCodeQueue::report_buffer_statistics() {
    SERIAL_ECHOLNPGM("D576"
                     ...
                     " (",
                     max_planner_buffer_empty_duration,
                     ")"
                     " B:",
                     BUFSIZE - ring_buffer.length,
                     " ",
                     -command_buffer_underruns,
                     " (",
                     max_command_buffer_empty_duration,
                     ")");
